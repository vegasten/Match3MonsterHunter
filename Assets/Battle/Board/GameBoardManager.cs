...
namespace Battle
{
    public class GameBoardManager : MonoBehaviour
    {
        ...
        public void StartTurn(Players attacker)
        {
            if (attacker == Players.Friendly)
            {
                BattleManager.IsBoardInputEnabled = true;
            }
            else
            {
                DoEnemyTurn();
        ...
        {
            (var tile1, var tile2) = FindEnemyPreferredTileSwitch();
            OnTileDragged(tile1, tile2);
        }

        private (Tile, Tile) FindEnemyPreferredTileSwitch()
        {
            var tile1 = _gameBoard.GetTileFromIndex(new Vector2Int(0, 0));
            var tile2 = _gameBoard.GetTileFromIndex(new Vector2Int(0, 1));

            while (!CanSwitch(tile1, tile2))
            {
                var randomX = UnityEngine.Random.Range(0, _numberOfColumns);
                var randomY = UnityEngine.Random.Range(0, _numberOfRows);

                tile1 = _gameBoard.GetTileFromIndex(new Vector2Int(randomX, randomY));

                switch (UnityEngine.Random.Range(0, 4))
                {
                    case 0:
                        if (randomX < _numberOfColumns - 1)
                            tile2 = _gameBoard.GetTileFromIndex(new Vector2Int(randomX + 1, randomY));
                        break;
                    case 1:
                        if (randomY < _numberOfRows - 1)
                            tile2 = _gameBoard.GetTileFromIndex(new Vector2Int(randomX, randomY + 1));
                        break;
                    case 2:
                        if (randomX > 0)
                            tile2 = _gameBoard.GetTileFromIndex(new Vector2Int(randomX - 1, randomY));
        ...
        {
            BattleManager.IsBoardInputEnabled = false;
